...
let fazerAposta = (valor) => new Promise((resolve, reject) => {
    ...
    if(saldo >= valor) {
        ...
        if(sorteio < 0.3) {
            ...
            resolve("OTÁRIO!!! Você perdeu R$" + valor + ".");
        }
        dinheiro.innerHTML = "Saldo: R$" + saldo;
    } else {
            reject("Você está duro e não pode apostar essa quantia. Aposte valores mais baixos para conseguir dinheiro.");
    }
...
btn.addEventListener("click", () => {
    odd = Math.floor(Math.random() * 0.5) + 1;
